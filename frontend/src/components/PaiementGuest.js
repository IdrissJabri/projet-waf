...
const PaiementGuest = ({
  ...
  const sendemail = () => {
    ...
    emailjs
      .send("gmail", "template_2ow9899", Email, "user_J2qecPjPjbUGndlTn6lGd")
      .then((result) => {
    ...
    try {
      const response = await fetch("http://localhost:8000/api/createGuest", {
        ...
        body: JSON.stringify({
          ...
          email,
          voyages_id,
          nombre_de_passagers: dataVille.psg,
          date_reservation: dataVille.date,
        }),
      });
      ...
      if (result.success) {
        ...
        setComplet(true);
        setConfirm(false);
        console.log("Guest reservation created successfully:", result);
      } else {
        console.error("Failed to create guest reservation:", result.message);
        alert(
          "Erreur lors de la création de la réservation: " + result.message
        );
      }
    } catch (error) {
  ...
  return (
    <div>
      <Modal show={displayguest} onHide={handleClose} className="ModalGuest">
        <Modal.Title>Paiement Visiteur</Modal.Title>
